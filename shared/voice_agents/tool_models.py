...
class PlatformToolCreate(PlatformToolBase):
    ...
    tool_functions_schema: Optional[Dict[str, Any]] = Field(None, description="Function schemas for LLM debugging and inspection")
    auth_type: Optional[str] = Field(None, description="Authentication type (e.g., 'oauth2', 'api_key')")
...
class AgentToolBase(BaseModel):
    ...
    unselected_functions: Optional[List[str]] = Field(None, description="Array of function names to disable (all others enabled by default)")
...
class AgentToolResponse(BaseModel):
    ...
    unselected_functions: Optional[List[str]] = Field(None, description="Array of function names to disable (all others enabled by default)")
    is_enabled: bool = Field(default=True, description="Whether the tool is enabled for this agent")
    auth_status: AuthStatus = Field(default=AuthStatus.NOT_AUTHENTICATED, description="Authentication status of the tool")
    token_expires_at: Optional[float] = Field(None, description="Timestamp when access token expires (Unix timestamp)")
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
    tool: Optional[PlatformTool] = Field(None, description="Tool details")
