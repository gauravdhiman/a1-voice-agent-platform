...
from src.rbac.permissions.models import (
    Permission,
    PermissionCreate,
    PermissionUpdate,
    RolePermission,
...
permission_operations_counter = meter.create_counter(
    "permission.operations", description="Number of permission operations"
)

permission_errors_counter = meter.create_counter(
    "permission.errors", description="Number of permission operation errors"
)
...
class PermissionService:
    ...
    async def create_permission(
        self, perm_data: PermissionCreate
        ...
        try:
            ...
            if not response.data:
                ...
                current_span.set_status(
                    trace.Status(trace.StatusCode.ERROR, "Failed to create permission")
                )
                permission_errors_counter.add(
                    1, {"operation": "create_permission", "error": "no_data_returned"}
                )
            ...
            logger.error(
                f"Exception while creating permission '{perm_data.name}': {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "create_permission", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_permission_by_id(
        ...
        try:
            ...
            if not response.data:
                ...
                permission_errors_counter.add(
                    1, {"operation": "get_permission_by_id", "error": "not_found"}
                )
            ...
            logger.error(
                f"Exception while getting permission {perm_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "get_permission_by_id", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_permission_by_name(
        ...
        try:
            ...
            if not response.data:
                ...
                permission_errors_counter.add(
                    1, {"operation": "get_permission_by_name", "error": "not_found"}
                )
                return None, "Permission not found"
            ...
            logger.error(
                f"Exception while getting permission '{name}': {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "get_permission_by_name", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_all_permissions(self) -> tuple[list[Permission], Optional[str]]:
        ...
        try:
            ...
            permission_errors_counter.add(
                1, {"operation": "get_all_permissions", "error": "exception"}
            )
            return [], str(e)

    @tracer.start_as_current_span("permission.update_permission")
    async def update_permission(
        self, perm_id: UUID, perm_data: PermissionUpdate
    ) -> tuple[Optional[Permission], Optional[str]]:
        """Update a permission."""
        ...
        try:
            ...
            if not response.data:
                ...
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Permission not found or update failed"
                    )
                )
                permission_errors_counter.add(
                    1,
                    {"operation": "update_permission", "error": "not_found_or_failed"},
                )
            ...
            logger.error(
                f"Exception while updating permission {perm_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "update_permission", "error": "exception"}
            )
            return None, str(e)
    ...
    async def delete_permission(self, perm_id: UUID) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            self.supabase.table("role_permissions").delete().eq(
            ...
            if not response.data:
                ...
                permission_errors_counter.add(
                    1, {"operation": "delete_permission", "error": "not_found"}
                )
            ...
            logger.error(
                f"Exception while deleting permission {perm_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "delete_permission", "error": "exception"}
            )
            return False, str(e)
    ...
    async def assign_permission_to_role(
        ...
        try:
            response = (
                ...
                .insert({"role_id": str(role_id), "permission_id": str(permission_id)})
                .execute()
            )
            ...
            if not response.data:
                ...
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Failed to assign permission to role"
                    )
                )
                permission_errors_counter.add(
                    1,
            ...
            logger.error(
                f"Exception while assigning permission {permission_id} to role {role_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "assign_permission_to_role", "error": "exception"}
            )
            return None, str(e)
    ...
    async def remove_permission_from_role(
        ...
        permission_operations_counter.add(
            1, {"operation": "remove_permission_from_role"}
        )
        ...
        try:
            response = (
                self.supabase.table("role_permissions")
                .delete()
                .match({"role_id": str(role_id), "permission_id": str(permission_id)})
                .execute()
            )

            if not response.data:
                logger.warning(
                    f"Role-permission assignment not found for role {role_id} and permission {permission_id}"
                )
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Role-permission assignment not found"
                    )
                )
                permission_errors_counter.add(
                    1,
                    {"operation": "remove_permission_from_role", "error": "not_found"},
                )
            ...
            logger.error(
                f"Exception while removing permission {permission_id} from role {role_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "remove_permission_from_role", "error": "exception"}
            )
            return False, str(e)
    ...
    async def get_permissions_for_role(
        ...
        try:
            ...
            logger.error(
                f"Exception while getting permissions for role {role_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            permission_errors_counter.add(
                1, {"operation": "get_permissions_for_role", "error": "exception"}
            )
            return [], str(e)
