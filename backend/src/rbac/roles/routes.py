...
async def create_role(
    ...
    if not user_profile.has_role("platform_admin"):
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Only platform administrators can create roles"
            )
        )
        raise HTTPException(
...
async def get_role(
    ...
    if not user_profile.has_permission("role:read"):
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Insufficient permissions to view roles"
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def update_role(
    ...
    if not user_profile.has_role("platform_admin"):
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Only platform administrators can update roles"
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def delete_role(
    ...
    if not user_profile.has_role("platform_admin"):
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Only platform administrators can delete roles"
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
