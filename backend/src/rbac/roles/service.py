...
role_operations_counter = meter.create_counter(
    "role.operations", description="Number of role operations"
)

role_errors_counter = meter.create_counter(
    "role.errors", description="Number of role operation errors"
)
...
class RoleService:
        ...
        try:
            ...
            if not response.data:
                ...
                current_span.set_status(
                    trace.Status(trace.StatusCode.ERROR, "Failed to create role")
                )
                role_errors_counter.add(
                    1, {"operation": "create_role", "error": "no_data_returned"}
                )
            ...
            logger.error(
                f"Exception while creating role '{role_data.name}': {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "create_role", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_role_by_id(
        ...
        try:
            ...
            if not response.data:
                ...
                role_errors_counter.add(
                    1, {"operation": "get_role_by_id", "error": "not_found"}
                )
            ...
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "get_role_by_id", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_role_by_name(self, name: str) -> tuple[Optional[Role], Optional[str]]:
        ...
        try:
            response = (
                self.supabase.table("roles").select("*").eq("name", name).execute()
            )
            ...
            if not response.data:
                ...
                role_errors_counter.add(
                    1, {"operation": "get_role_by_name", "error": "not_found"}
                )
                return None, "Role not found"
            ...
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "get_role_by_name", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_all_roles(self) -> tuple[list[Role], Optional[str]]:
        ...
        try:
            ...
            role_errors_counter.add(
                1, {"operation": "get_all_roles", "error": "exception"}
            )
            return [], str(e)
    ...
    async def update_role(
        ...
        try:
            ...
            if not response.data:
                ...
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Role not found or update failed"
                    )
                )
                role_errors_counter.add(
                    1, {"operation": "update_role", "error": "not_found_or_failed"}
                )
            ...
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "update_role", "error": "exception"}
            )
            return None, str(e)
    ...
    async def delete_role(self, role_id: UUID) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            self.supabase.table("user_roles").delete().eq(
                "role_id", str(role_id)
            ).execute()

            # Finally delete the role
            response = (
                self.supabase.table("roles").delete().eq("id", str(role_id)).execute()
            )
            ...
            if not response.data:
                ...
                role_errors_counter.add(
                    1, {"operation": "delete_role", "error": "not_found"}
                )
            ...
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "delete_role", "error": "exception"}
            )
            return False, str(e)
    ...
    async def get_role_with_permissions(
        self, role_id: UUID
    ) -> tuple[Optional[RoleWithPermissions], Optional[str]]:
        ...
        try:
            ...
            if error or not role:
                current_span.set_status(
                    trace.Status(trace.StatusCode.ERROR, error or "Role not found")
                )
                role_errors_counter.add(
                    1,
                    {
            ...
            logger.error(
                f"Exception while getting role with permissions {role_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            role_errors_counter.add(
                1, {"operation": "get_role_with_permissions", "error": "exception"}
            )
            return None, str(e)
