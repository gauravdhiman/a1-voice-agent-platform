...
@user_role_router.post(
    "/user-roles", response_model=UserRole, status_code=status.HTTP_201_CREATED
)
...
async def assign_role_to_user(
    ...
    if not is_authorized:
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Insufficient permissions to assign roles"
            )
        )
        raise HTTPException(
...
async def update_user_role(
    ...
    if not user_profile.has_role("platform_admin"):
        current_span.set_status(
            trace.Status(
                ...
                "Only platform administrators can update user role assignments",
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    user_role, error = await user_role_service.update_user_role(
        user_role_id, user_role_data
    )
    if error:
...
@user_role_router.delete(
    "/user-roles/{user_role_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
async def remove_role_from_user(
    ...
    if not user_profile.has_role("platform_admin"):
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR,
                "Only platform administrators can remove user role assignments",
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_user_roles(
    ...
    if not is_authorized:
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR, "Insufficient permissions to view user roles"
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@user_role_router.get(
    "/users/{user_id}/roles-with-permissions", response_model=list[RoleWithPermissions]
)
...
async def get_user_roles_with_permissions(
    ...
    if not is_authorized:
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR,
                "Insufficient permissions to view user roles with permissions",
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if user_id == current_user_id:
        ...
        # For other users, make DB call
        current_span.set_attribute("optimization.used", False)
        roles_with_permissions, error = (
            await user_role_service.get_user_roles_with_permissions(
...
@user_role_router.get(
    "/users/{user_id}/permissions/{permission_name}", response_model=bool
)
...
async def check_user_permission(
    ...
    has_permission, error = await user_role_service.user_has_permission(
...
async def check_user_role(
    ...
    if not is_authorized:
        current_span.set_status(
            trace.Status(
                trace.StatusCode.ERROR,
                "Insufficient permissions to check other users' roles",
            )
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    has_role, error = await user_role_service.user_has_role(
