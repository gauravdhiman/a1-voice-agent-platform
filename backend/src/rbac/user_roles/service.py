...
from src.organization.member_models import MemberRole, OrganizationMember
...
from src.rbac.user_roles.models import (
    UserRole,
    UserRoleCreate,
    UserRoleUpdate,
    UserWithRoles,
...
user_role_operations_counter = meter.create_counter(
    "user_role.operations", description="Number of user role operations"
)

user_role_errors_counter = meter.create_counter(
    "user_role.errors", description="Number of user role operation errors"
)
...
class UserRoleService:
    ...
    async def assign_role_to_user(
        self, user_role_data: UserRoleCreate
        ...
        if user_role_data.organization_id:
            current_span.set_attribute(
                "organization.id", str(user_role_data.organization_id)
            )
        try:
            insert_data = {
            ...
            if not response.data:
                logger.error(
                    f"Failed to assign role {user_role_data.role_id} to user {user_role_data.user_id}"
                )
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Failed to assign role to user"
                    )
                )
                user_role_errors_counter.add(
                    1, {"operation": "assign_role_to_user", "error": "no_data_returned"}
                )
            ...
            logger.error(
                f"Exception while assigning role {user_role_data.role_id} to user {user_role_data.user_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "assign_role_to_user", "error": "exception"}
            )
            return None, str(e)

    @tracer.start_as_current_span("user_role.update_user_role")
    async def update_user_role(
        self, user_role_id: UUID, user_role_data: UserRoleUpdate
    ) -> tuple[Optional[UserRole], Optional[str]]:
        ...
        try:
            ...
            if user_role_data.organization_id is not None:
                update_data["organization_id"] = (
                    str(user_role_data.organization_id)
                    if user_role_data.organization_id
                    else None
                )
            ...
            if not response.data:
                logger.error(
                    f"User role assignment not found or update failed: {user_role_id}"
                )
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR,
                        "User role assignment not found or update failed",
                    )
                )
                user_role_errors_counter.add(
                    1, {"operation": "update_user_role", "error": "not_found_or_failed"}
                )
            ...
            logger.error(
                f"Exception while updating user role {user_role_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "update_user_role", "error": "exception"}
            )
            return None, str(e)

    @tracer.start_as_current_span("user_role.remove_role_from_user")
    async def remove_role_from_user(
        self, user_role_id: UUID
        ...
        try:
            ...
            if not response.data:
                logger.warning(
                    f"User role assignment not found for deletion: {user_role_id}"
                )
                current_span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "User role assignment not found"
                    )
                )
                user_role_errors_counter.add(
                    1, {"operation": "remove_role_from_user", "error": "not_found"}
                )
            ...
            logger.error(
                f"Exception while removing role from user {user_role_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "remove_role_from_user", "error": "exception"}
            )
            return False, str(e)
    ...
    async def get_user_role_by_id(
        ...
        try:
            ...
            if not response.data:
                ...
                user_role_errors_counter.add(
                    1, {"operation": "get_user_role_by_id", "error": "not_found"}
                )
            ...
            logger.error(
                f"Exception while getting user role {user_role_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "get_user_role_by_id", "error": "exception"}
            )
            return None, str(e)
    ...
    async def get_user_roles(
        ...
        try:
            ...
            logger.error(
                f"Exception while getting roles for user {user_id}: {e}", exc_info=True
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "get_user_roles", "error": "exception"}
            )
            return [], str(e)
    ...
    async def get_user_roles_with_permissions(
        self, user_id: UUID, organization_id: Optional[UUID] = None
    ) -> tuple[list[RoleWithPermissions], Optional[str]]:
        """Get all roles with their permissions for a user."""
        user_role_operations_counter.add(
            1, {"operation": "get_user_roles_with_permissions"}
        )
        ...
        try:
            ...
            if error:
                ...
                user_role_errors_counter.add(
                    1,
                    {
                        "operation": "get_user_roles_with_permissions",
            ...
            for role in roles:
                ...
                permissions, error = await permission_service.get_permissions_for_role(
                    role.id
                )
                if error:
                    logger.error(
                        f"Error getting permissions for role {role.id} for user {user_id}: {error}"
                    )
                    current_span.set_status(trace.Status(trace.StatusCode.ERROR, error))
                    user_role_errors_counter.add(
                        1,
            ...
            current_span.set_attribute(
                "roles_with_permissions.count", len(roles_with_permissions)
            )
            current_span.set_status(trace.Status(trace.StatusCode.OK))
            ...
            logger.error(
                f"Exception while getting roles with permissions for user {user_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1,
                {"operation": "get_user_roles_with_permissions", "error": "exception"},
            )
            return [], str(e)

    @tracer.start_as_current_span("user_role.get_all_user_roles_with_permissions")
    async def get_all_user_roles_with_permissions(
        self, user_id: UUID
    ) -> tuple[list[UserRoleWithPermissions], Optional[str]]:
        """Get all roles with their permissions and organization context for a user."""
        user_role_operations_counter.add(
            1, {"operation": "get_all_user_roles_with_permissions"}
        )
        ...
        try:
            ...
            for role_id in role_ids:
                ...
                role_with_perms, error = await role_service.get_role_with_permissions(
                    role_id
                )
                if error:
                    logger.error(
                        f"Error getting role with permissions for role {role_id}: {error}"
                    )
                    continue
            ...
            logger.error(
                f"Exception while getting all roles with permissions for user {user_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1,
                {
                    "operation": "get_all_user_roles_with_permissions",
                    "error": "exception",
                },
            )
            return [], str(e)
    ...
    async def user_has_permission(
        ...
        try:
            ...
            response = (
                self.supabase.table("permissions")
                .select("*")
            ...
            if not response.data:
                current_span.set_status(
                    trace.Status(trace.StatusCode.ERROR, "Permission not found")
                )
                user_role_errors_counter.add(
                    1,
                    {
            ...
            if error:
                ...
                current_span.set_status(trace.Status(trace.StatusCode.ERROR, error))
                user_role_errors_counter.add(
                    1, {"operation": "user_has_permission", "error": "get_roles_failed"}
                )
                return False, error
            ...
            for role in roles:
                response = (
                    ...
                    .select("*")
                    .match({"role_id": str(role.id), "permission_id": permission_id})
                    .execute()
                )
            ...
            logger.error(
                f"Exception while checking permission '{permission_name}' for user {user_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "user_has_permission", "error": "exception"}
            )
            return False, str(e)
    ...
    async def user_has_role(
        ...
        try:
            ...
            response = (
                self.supabase.table("roles").select("*").eq("name", role_name).execute()
            )

            if not response.data:
                current_span.set_status(
                    trace.Status(trace.StatusCode.ERROR, "Role not found")
                )
                user_role_errors_counter.add(
                    1, {"operation": "user_has_role", "error": "role_not_found"}
                )
            ...
            query = (
                self.supabase.table("user_roles")
                .select("*")
                .match({"user_id": str(user_id), "role_id": role_id})
            )
            ...
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "user_has_role", "error": "exception"}
            )
            return False, str(e)
    ...
    async def get_users_by_organization(
    ...
    ) -> tuple[list[OrganizationMember], Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                while not all_users_found:
                    try:
                        ...
                        if hasattr(users_response, "users"):
                            ...
                            logger.warning(
                                f"Unknown response format for list_users: {type(users_response)}"
                            )
                            break
                        ...
                        logger.warning(
                            f"Error in list_users pagination (page {page}): {e}"
                        )
                        break
                ...
                if missing_user_ids:
                    logger.info(
                        f"Fetching {len(missing_user_ids)} missing users individually"
                    )
                    for user_id in missing_user_ids:
                        try:
                            ...
                            logger.warning(
                                f"Could not fetch missing user {user_id}: {e}"
                            )
                            continue
            ...
            for user_id, roles in user_roles_map.items():
                ...
                if user_data:
                    member_data = {**user_data, "roles": roles}
                    members.append(OrganizationMember(**member_data))

            logger.info(
                f"Found {len(members)} organization members for organization {organization_id}"
            )
            ...
            logger.error(
                f"Exception while getting users for organization {organization_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "get_users_by_organization", "error": "exception"}
            )
            return [], str(e)
    ...
    async def get_organizations_for_user(
        ...
        try:
            ...
            logger.error(
                f"Exception while getting organizations for user {user_id}: {e}",
                exc_info=True,
            )
            current_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            user_role_errors_counter.add(
                1, {"operation": "get_organizations_for_user", "error": "exception"}
            )
            return [], str(e)
