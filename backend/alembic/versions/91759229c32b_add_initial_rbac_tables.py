...
def downgrade() -> None:
    # Drop RLS policies
    op.execute(
        'DROP POLICY IF EXISTS "Org admins can manage user roles within their organization" ON user_roles'
    )
    op.execute(
    ...
    op.execute(
        'DROP POLICY IF EXISTS "Platform admins can view all permissions" ON permissions'
    )
