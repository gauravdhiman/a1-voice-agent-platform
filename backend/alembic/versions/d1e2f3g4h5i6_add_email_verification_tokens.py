...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "user_id", sa.UUID(), nullable=False, comment="Reference to auth.users.id"
        ),
        sa.Column(
            "token",
        ...
        sa.UniqueConstraint(
            "user_id", name="uq_email_verification_tokens_user_id"
        ),  # One token per user
    )
    ...
    op.create_index(
        "idx_email_verification_tokens_token", "email_verification_tokens", ["token"]
    )
    op.create_index(
...
def downgrade() -> None:
    # Drop RLS policies
    op.execute(
        'DROP POLICY IF EXISTS "Users can delete own email verification tokens" ON email_verification_tokens'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Users can update own email verification tokens" ON email_verification_tokens'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Users can insert own email verification tokens" ON email_verification_tokens'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Users can view own email verification tokens" ON email_verification_tokens'
    )
