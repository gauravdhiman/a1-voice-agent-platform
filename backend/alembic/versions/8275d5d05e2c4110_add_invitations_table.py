...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_invitations_organization_id", "invitations", ["organization_id"]
    )
...
def downgrade() -> None:
    ...
    # Drop RLS policies
    op.execute(
        'DROP POLICY IF EXISTS "Organization admins can delete invitations for their organization" ON invitations'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Organization admins can update invitations for their organization" ON invitations'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Organization admins can insert invitations for their organization" ON invitations'
    )
    op.execute(
        'DROP POLICY IF EXISTS "Organization admins can view invitations for their organization" ON invitations'
    )
