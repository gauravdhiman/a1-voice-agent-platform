...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "price_amount", sa.INTEGER(), nullable=False, comment="Price in cents"
        ),
        sa.Column(
            "currency", sa.VARCHAR(length=3), nullable=False, server_default="USD"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.VARCHAR(length=50), nullable=False
    ...
    op.create_table(
        ...
        sa.Column(
            "category", sa.VARCHAR(length=50), nullable=False
        ),  # api_call, storage, compute, etc.
        sa.Column(
            "is_active", sa.BOOLEAN(), server_default=sa.text("true"), nullable=False
        ),
        sa.Column("metadata", postgresql.JSONB(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("organization_id", sa.UUID(), nullable=False),
        sa.Column(
            "transaction_type", sa.VARCHAR(length=50), nullable=False
        ),  # earned, purchased, consumed, expired
        sa.Column(
            "amount", sa.INTEGER(), nullable=False
        ),  # positive for credits added, negative for consumed
        sa.Column("balance_after", sa.INTEGER(), nullable=False),
        sa.Column(
            "source", sa.VARCHAR(length=50), nullable=False
        ),  # subscription, purchase, event_consumption, expiry
        sa.Column(
            "source_id", sa.UUID(), nullable=True
        ),  # reference to subscription, purchase, or event
        sa.Column(
            "credit_event_id", sa.UUID(), nullable=True
        ...
        sa.ForeignKeyConstraint(
            ["credit_event_id"], ["credit_events.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "billing_reason", sa.VARCHAR(length=50), nullable=True
    ...
    op.create_index(
        "idx_credit_transactions_org_id", "credit_transactions", ["organization_id"]
    )
    op.create_index(
        "idx_credit_transactions_created_at", "credit_transactions", ["created_at"]
    )
    op.create_index(
        "idx_billing_history_org_id", "billing_history", ["organization_id"]
    )
    op.create_index(
