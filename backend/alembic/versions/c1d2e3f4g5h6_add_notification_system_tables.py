...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=False,
            comment="Unique identifier for the event (e.g., user.signup, subscription.created)",
        ),
        sa.Column(
            "category",
            sa.VARCHAR(length=50),
            nullable=False,
            comment="Event category (e.g., auth, billing, organization)",
        ),
        sa.Column(
        ...
        sa.Column(
            ...
            sa.UUID(),
            nullable=True,
            comment="Default template to use for this event",
        ),
        sa.Column(
            "metadata",
            postgresql.JSONB(),
            nullable=True,
            comment="Additional configuration (e.g., recipients, conditions)",
        ),
        sa.Column(
            "created_at",
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.TEXT(),
            nullable=False,
            comment="HTML email template with placeholders",
        ),
        sa.Column(
            "text_content", sa.TEXT(), nullable=True, comment="Plain text fallback"
        ),
        sa.Column(
            "from_email",
            sa.VARCHAR(length=255),
            nullable=True,
            comment="Override sender email",
        ),
        sa.Column(
            "from_name",
            sa.VARCHAR(length=100),
            nullable=True,
            comment="Override sender name",
        ),
        sa.Column(
            "template_variables",
            postgresql.JSONB(),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.UUID(),
            nullable=True,
            comment="User who triggered or is recipient",
        ),
        ...
        sa.Column(
            ...
            postgresql.JSONB(),
            nullable=True,
            comment="Additional data (variables used, context)",
        ),
        sa.Column(
            "created_at",
            postgresql.TIMESTAMP(timezone=True),
    ...
    op.create_index(
        "idx_notification_events_event_key", "notification_events", ["event_key"]
    )
    op.create_index(
        "idx_notification_events_category", "notification_events", ["category"]
    )
    op.create_index(
        "idx_notification_events_is_enabled", "notification_events", ["is_enabled"]
    )
    op.create_index(
        "idx_notification_templates_is_active", "notification_templates", ["is_active"]
    )
    op.create_index(
        "idx_notification_logs_event_id", "notification_logs", ["notification_event_id"]
    )
    op.create_index(
    ...
    op.create_index(
        "idx_notification_logs_created_at", "notification_logs", ["created_at"]
    )
    op.create_index(
