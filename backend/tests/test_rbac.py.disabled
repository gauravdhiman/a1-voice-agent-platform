...
import pytest
from src.auth.rbac_models import PermissionCreate, RoleCreate, UserRoleCreate
...
class TestRBAC:
    ...
    async def test_create_role(self):
        ...
        role_data = RoleCreate(name="test_role", description="A test role")
    ...
    async def test_assign_role_to_user(self):
        ...
        role_data = RoleCreate(
            name="test_user_role", description="A test role for users"
        )
        ...
        user_role_data = UserRoleCreate(
            user_id=user_id, role_id=role.id, organization_id=None
        )
