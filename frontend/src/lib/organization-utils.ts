...
export function isDummyOrganization(org: OrganizationForDetection): boolean {
  ...
  return (
    ...
    (org.description &&
      org.description.toLowerCase().includes(defaultKeyword)) ||
 ...
 */
export function createDummyOrganizationData(user: {
  firstName?: string;
  email?: string;
  id?: string;
}) {
