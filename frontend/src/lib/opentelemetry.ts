...
function shouldExcludeSpan(spanName: string): boolean {
  ...
  const excludedPatterns = ["GET (GET /favicon.ico"];
...
const hasSetupDiagLogger = (global as unknown as Record<string, unknown>)
  .__OTEL_DIAG_LOGGER_SETUP__;

// Enable OpenTelemetry diagnostics for debugging (only once)
if (!hasSetupDiagLogger) {
  diag.setLogger(new DiagConsoleLogger(), DiagLogLevel.DEBUG);
  (global as unknown as Record<string, boolean>).__OTEL_DIAG_LOGGER_SETUP__ =
    true;
}
...
export function initOpenTelemetry() {
  ...
  if (typeof window === "undefined") {
    console.log(
      "Skipping OpenTelemetry initialization during Next.js build (Node.js environment)",
    );
    isInitialized = true;
    return;
  }
  ...
  try {
    ...
    const resource = new Resource({
      [SemanticResourceAttributes.SERVICE_NAME]:
        process.env.NEXT_PUBLIC_OTEL_SERVICE_NAME || "saas-platform-frontend",
    ...
    traceExporter.export = function (spans, resultCallback) {
      ...
      const filteredSpans = spans.filter((span) => {
        ...
        const spanName =
          "name" in span ? (span as { name: string }).name : "Unknown Span";
