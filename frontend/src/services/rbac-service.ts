...
class RBACService {
  ...
  async updateUserRole(
    userRoleId: string,
    userRoleData: Partial<
      Omit<UserRole, "id" | "user_id" | "created_at" | "updated_at">
    >,
  ): Promise<UserRole> {
  ...
  async userHasPermission(
    ...
    const response = await apiClient.get<{ has_permission: boolean }>(
      `${this.baseUrl}/users/${userId}/has-permission?${params}`,
    );
  ...
  async userHasRole(
    ...
    if (organizationId) params.append("organization_id", organizationId);
    const response = await apiClient.get<{ has_role: boolean }>(
      `${this.baseUrl}/users/${userId}/has-role?${params}`,
    );
